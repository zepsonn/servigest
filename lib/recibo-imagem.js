...
export async function gerarReciboPNG(dados, familiaFonte) {
  ...
  // "partes" (com tipo titulo/item) monta o agrupamento por aparelho.
  ;[...(dados.listas || [])].forEach(L2 => {
    const partes = L2.partes || (L2.itens || []).map(t => ({ tipo: 'item', texto: t }))
    if (!partes.length) return
    const temTitulo = partes.some(p => p.tipo === 'titulo')
    const nItens = partes.filter(p => p.tipo === 'item').length
    const desenho = []
    let nLinhas = 0
    partes.forEach(p => {
      const titulo = p.tipo === 'titulo'
      medidor.font = fonte(titulo || L2.forte ? 700 : 500, 25, familiaFonte)
      const prefixo = (!titulo && (temTitulo || nItens > 1)) ? '•  ' : ''
      const linhas = quebrar(medidor, prefixo + p.texto, larguraTexto)
      desenho.push({ titulo, linhas })
      nLinhas += linhas.length
    })
    const espacos = temTitulo ? partes.filter(p => p.tipo === 'titulo').length * 10 : 0
    const alt = 20 + 17 + nLinhas * 34 + espacos + 20
    const rotulo = temTitulo ? L2.varios : (nItens > 1 ? L2.varios : L2.um)
    blocos.push({ tipo: 'lista', rotulo, desenho, alt, forte: L2.forte })
    alturaCorpo += alt
  })

  // varios aparelhos na mesma OS: um bloco por aparelho — nome em destaque,
  // servicos daquele aparelho embaixo e o valor dele na direita (se tiver).
  const aps = (dados.aparelhos || []).filter(a => String(a.nome || '').trim() || String(a.servico || '').trim())
  if (aps.length) {
    const desenho = []
    let altura = 0
    aps.forEach((a, i) => {
      const val = Number(a.valor) || 0
      const rotuloVal = val > 0 ? val.toLocaleString('pt-BR', { style:'currency', currency:'BRL' }) : ''
      medidor.font = fonte(800, 25, familiaFonte)
      const largVal = rotuloVal ? medidor.measureText(rotuloVal).width + 24 : 0
      medidor.font = fonte(800, 27, familiaFonte)
      const nomeLinhas = quebrar(medidor, a.nome || ('Aparelho ' + (i + 1)), larguraTexto - 16 - largVal)
      medidor.font = fonte(500, 24, familiaFonte)
      const itensLinhas = []
      itensDe(a.servico).forEach(s => { itensLinhas.push(...quebrar(medidor, '•  ' + s, larguraTexto - 26)) })
      const gap = i ? 18 : 0
      const alt = gap + nomeLinhas.length * 36 + itensLinhas.length * 32
      desenho.push({ nomeLinhas, itensLinhas, valor: rotuloVal, gap })
      altura += alt
    })
    const alt = 20 + 17 + altura + 22
    blocos.push({ tipo: 'aparelhos', rotulo: 'Aparelhos e serviços', desenho, alt })
    alturaCorpo += alt
  }
  ...
  blocos.forEach((b, i) => {
    ...
    ctx.fillStyle = TINTA
    if (b.tipo === 'lista') {
      b.desenho.forEach((d, k) => {
        if (d.titulo && k > 0) yy += 10          // respiro antes de cada aparelho
        ctx.font = fonte(d.titulo || b.forte ? 700 : 500, 25, familiaFonte)
        d.linhas.forEach(l => { ctx.fillText(l, PAD, yy); yy += 34 })
      })
    } else if (b.tipo === 'aparelhos') {
      b.desenho.forEach(d => {
        yy += d.gap
        const yIni = yy
        ctx.fillStyle = TINTA
        ctx.font = fonte(800, 27, familiaFonte)
        d.nomeLinhas.forEach(l => { ctx.fillText(l, PAD + 18, yy); yy += 36 })
        if (d.valor) {
          ctx.textAlign = 'right'
          ctx.fillStyle = VERDE
          ctx.font = fonte(800, 25, familiaFonte)
          ctx.fillText(d.valor, L - PAD, yIni + 2)
          ctx.textAlign = 'left'
        }
        ctx.fillStyle = TINTA
        ctx.font = fonte(500, 24, familiaFonte)
        d.itensLinhas.forEach(l => { ctx.fillText(l, PAD + 18, yy); yy += 32 })
        // barrinha verde na esquerda, igual ao recibo da tela
        ctx.strokeStyle = VERDE; ctx.lineWidth = 4; ctx.lineCap = 'round'
        ctx.beginPath(); ctx.moveTo(PAD + 3, yIni + 5); ctx.lineTo(PAD + 3, yy - 9); ctx.stroke()
        ctx.lineCap = 'butt'
      })
    } else {
