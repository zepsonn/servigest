...
export async function gerarReciboPNG(dados, familiaFonte) {
  ...
  if ((dados.servicos || []).length) {
    medidor.font = fonte(500, 25, familiaFonte)
    const itens = dados.servicos.map(s => quebrar(medidor, (dados.servicos.length > 1 ? '•  ' : '') + s, larguraTexto))
    const nLinhas = itens.reduce((s, i) => s + i.length, 0)
    const alt = 20 + 17 + nLinhas * 34 + 20
    blocos.push({ tipo: 'servicos', rotulo: dados.servicos.length > 1 ? 'SERVIÇOS REALIZADOS' : 'SERVIÇO REALIZADO', itens, alt })
    alturaCorpo += alt
  }
  ...
  // circulo com o check
  const cx = L / 2
  ctx.beginPath(); ctx.arc(cx, y + 48, 48, 0, Math.PI * 2)
  ctx.fillStyle = 'rgba(255,255,255,.2)'; ctx.fill()
  ctx.strokeStyle = '#fff'; ctx.lineWidth = 5.5; ctx.lineCap = 'round'; ctx.lineJoin = 'round'
  ctx.beginPath(); ctx.moveTo(cx - 20, y + 48); ctx.lineTo(cx - 6, y + 62); ctx.lineTo(cx + 21, y + 34); ctx.stroke()
  y += 96 + 26
  ...
  blocos.forEach((b, i) => {
    ...
    ctx.fillStyle = TINTA
    if (b.tipo === 'servicos') {
      ctx.font = fonte(500, 25, familiaFonte)
