...
// gradiente de cada secao — usado no menu (item ativo) e nos icones
export const GRADIENTES = {
  dashboard:    ['#a955ff', '#ea51ff'],
  os:           ['#56CCF2', '#2F80ED'],
  clientes:     ['#FF9966', '#FF5E62'],
  etiqueta:     ['#f6d365', '#f5576c'],
  importar:     ['#80FF72', '#25D366'],
  estoque:      ['#f7b733', '#fc4a1a'],
  vendas:       ['#43e97b', '#38f9d7'],
  recibo:       ['#4facfe', '#00f2fe'],
  recibovenda:  ['#30cfd0', '#330867'],
  faturamento:  ['#f6d365', '#fda085'],
  despesas:     ['#ff758c', '#ff7eb3'],
  funcionarios: ['#a18cd1', '#fbc2eb'],
}
export const grad = (nome, dir = '135deg') => {
  const g = GRADIENTES[nome] || GRADIENTES.dashboard
  return 'linear-gradient(' + dir + ',' + g[0] + ',' + g[1] + ')'
}
...
export function ThemeProvider({ children }) {
  ...
  const t = {
    ...
    // no escuro a profundidade vem do CONTRASTE (card mais claro que o fundo),
    // porque sombra preta sobre fundo preto nao aparece
    bg:        dark ? '#0e0d0b' : '#f6f5f2',
    bgCard:    dark ? '#1c1b17' : '#ffffff',
    bgSidebar: dark ? '#141310' : '#fbfaf7',
    bgInput:   dark ? '#232119' : '#ffffff',
    bgHover:   dark ? '#252319' : '#efede8',
    text:      dark ? '#f2f0eb' : '#1c1b18',
    textSoft:  dark ? '#aca79b' : '#6b675f',
    border:    dark ? '#35322b' : '#e3e0d9',
    borderSoft:dark ? '#282620' : '#eceae4',
    ...
    // cards flutuantes — mais profundidade, pra destaque
    shadowFloat: dark ? '0 2px 4px rgba(0,0,0,.4), 0 20px 45px -20px rgba(0,0,0,.7)'
                      : '0 2px 4px rgba(28,27,24,.04), 0 18px 40px -18px rgba(28,27,24,.22)',
    radius: 16,
  }
