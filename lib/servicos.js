...
// Tipos de aparelho — usado pra reconhecer quando uma linha do campo de
// servico e, na verdade, o nome do produto (cabecalho) e nao um servico.
const APARELHOS = [
  'geladeira','refrigerador','freezer','frigobar','expositor',
  'lavadora','lava e seca','lava-seca','lava seca','maquina de lavar','máquina de lavar','tanquinho','secadora',
  'micro-ondas','microondas','micro ondas',
  'fogao','fogão','cooktop','forno','coifa','depurador',
  'ar condicionado','ar-condicionado','adega','purificador','bebedouro','climatizador',
]

const semAcento = s => String(s || '').normalize('NFD').replace(/[̀-ͯ]/g, '').toLowerCase().trim()

/**
 * Separa o campo de servico em titulos (nome do aparelho) e itens (o servico).
 *
 * Reconhece um titulo de duas formas:
 *   1. linha terminando com ":"        -> "Lavadora Brastemp:"
 *   2. linha TODA EM MAIUSCULA que comeca com um tipo de aparelho
 *      -> "LAVADORA BRASTEMP", "SECADORA ELECTROLUX", "GELADEIRA"
 *
 * A regra 2 e o que faz as OS ja cadastradas funcionarem sem redigitar.
 * "REVISAO GERAL" continua sendo item: e maiuscula, mas nao e aparelho.
 */
export function agruparServicos(texto) {
  const linhas = String(texto || '').split('\n')
    .map(s => s.trim().replace(/^[-•*]\s*/, ''))
    .filter(Boolean)

  return linhas.map(linha => {
    const semDoisPontos = linha.replace(/\s*:\s*$/, '')
    const ehTitulo =
      /:\s*$/.test(linha) ||
      (linha === linha.toUpperCase() &&
       linha.replace(/[^A-Za-zÀ-ÿ]/g, '').length >= 3 &&
       APARELHOS.some(a => semAcento(linha).startsWith(semAcento(a))))
    return { tipo: ehTitulo ? 'titulo' : 'item', texto: semDoisPontos }
  })
}
