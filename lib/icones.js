...
/**
 * Selo com o numero da OS — "#412".
 *
 * Fica em TODO card/linha de ordem de servico pra dar pra achar o pedido
 * rapido (e pra saber qual OS esta aberta na mao). Numero tabular pra os
 * digitos alinharem entre uma linha e outra.
 *
 * tom="destaque" usa a cor de acento — pro cabecalho dos paineis e dos
 * cards grandes, onde o numero e a informacao principal.
 */
export function NumOS({ n, t, tom = 'neutro', size = 11 }) {
  if (n == null || n === '') return null
  const destaque = tom === 'destaque'
  return (
    <span title={'Ordem de serviço nº ' + n}
      style={{
        display: 'inline-flex', alignItems: 'center', flexShrink: 0,
        padding: size >= 13 ? '3px 9px' : '2px 7px', borderRadius: 7,
        fontSize: size, fontWeight: 800, lineHeight: 1.5, letterSpacing: '.01em',
        fontVariantNumeric: 'tabular-nums',
        background: destaque ? t.accentSoft : t.bgSidebar,
        border: '1px solid ' + (destaque ? t.accent : t.borderSoft),
        color: destaque ? t.accentDark : t.textSoft,
      }}>
      <span style={{ opacity: .6, marginRight: 1 }}>#</span>{n}
    </span>
  )
}
