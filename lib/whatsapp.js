 ...
 */
import { lerAparelhos, itensDe } from './aparelhos'
...
export function textoOS(o) {
  const servicos = String(o.servico || '').split('\n').map(s => limpo(s).replace(/^[-•]\s*/, ''))
    .filter(s => s && !/:$/.test(s))       // tira os cabecalhos de aparelho
  const quando = [fmtDataBR(o.data_entrada), o.periodo ? (PERIODO_LABEL[o.periodo] || o.periodo) : null]
    .filter(Boolean).join(' · ')
  const mapa = linkMapa(o)

  // mais de um aparelho na OS: lista cada um com os servicos dele embaixo
  const aps = lerAparelhos(o).filter(a => limpo(a.nome) || limpo(a.servico))
  const varios = aps.length > 1
  const blocoAparelhos = varios
    ? '❄️ *Aparelhos:*\n' + aps.map((a, i) => {
        const itens = itensDe(a.servico).map(s => '   • ' + limpo(s))
        return ['*' + (limpo(a.nome) || ('Aparelho ' + (i + 1))) + '*', ...itens].join('\n')
      }).join('\n')
    : null
