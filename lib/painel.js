...
import { NumOS } from './icones'

export function Painel({ aberto, onFechar, titulo, subtitulo, numero, children, t, maxWidth = 520 }) {
  ...
  return createPortal(
    <div onClick={onFechar}
      ...
      <div onClick={e => e.stopPropagation()} className="sg-sheet"
        ...
        {(titulo || subtitulo || numero != null) && (
          <div style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 9, flexWrap: 'wrap' }}>
              {titulo && <div style={{ fontSize: 17, fontWeight: 700, color: t.text }}>{titulo}</div>}
              <NumOS n={numero} t={t} tom="destaque" size={12} />
            </div>
            {subtitulo && <div style={{ fontSize: 13, color: t.textSoft, marginTop: 4 }}>{subtitulo}</div>}
          </div>
        )}
        {children}
      </div>
