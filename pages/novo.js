...
export default function NovoAgendamento() {
  ...
  const [form, setForm] = useState({
    ...
    funcionario_id:'', servico:'', data:'', hora:'', status:'em_andamento', valor:0, observacoes:''
  })
      ...
      <div style={s.box}>
        ...
        <div style={{marginBottom:14}}>
          ...
          <div style={{display:'flex',gap:8}}>
            ...
            <button type="button" style={{padding:'0 14px',borderRadius:8,border:'1px solid '+t.accent,background:t.accentSoft,color:t.accentDark,fontSize:12,cursor:'pointer',fontWeight:600,whiteSpace:'nowrap'}} onClick={()=>setCliModal(true)}>+ Novo cliente</button>
          </div>
        </div>
        <div style={s.row2}>
        ...
        <div style={s.row2}>
          ...
          <FG label="Hora *" value={form.hora} onChange={v=>setForm({...form,hora:v})} t={t} type="time"/>
        </div>
        <div style={s.row2}>
        ...
        <div style={{marginBottom:14}}><label style={s.label}>Funcionário responsável</label>
        ...
        <div style={{fontSize:11,color:t.textSoft,marginTop:4}}>Ao salvar, este agendamento também cria uma Ordem de Serviço automaticamente.</div>
      ...
      {cliModal && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.55)',zIndex:100,display:'flex',alignItems:'center',justifyContent:'center'}}>
          <div style={{background:t.bgCard,borderRadius:12,padding:24,width:440,maxWidth:'96vw',border:'1px solid '+t.border}}>
            <div style={{fontSize:15,fontWeight:600,marginBottom:16,display:'flex',justifyContent:'space-between',color:t.text}}><span>Cadastrar novo cliente</span><button style={{background:'none',border:'none',fontSize:20,cursor:'pointer',color:t.textSoft}} onClick={()=>setCliModal(false)}>×</button></div>
            ...
            <div style={{display:'flex',gap:8,justifyContent:'flex-end',marginTop:16}}>
              ...
              <button style={s.btnPrimary} onClick={salvarCliente}>Cadastrar e usar</button>
            </div>
          </div>
        </div>
      )}
