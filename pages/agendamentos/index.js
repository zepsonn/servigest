...
export default function Agendamentos() {
  ...
  async function apagar(ag) {
    if(!confirm('Apagar agendamento de "'+(ag.clientes?.nome||'cliente')+'"?')) return
      ...
      <div style={s.card}>
        <table style={s.table}>
          ...
          <tbody>{agendamentos.map(a=>(
            <tr key={a.id}>
              <td style={s.td}><div style={{fontWeight:500,color:t.text}}>{a.clientes?.nome}</div><div style={{fontSize:11,color:t.textSoft}}>{a.clientes?.telefone}</div></td>
