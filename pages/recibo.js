...
export default function Recibo() {
  ...
  // varias linhas viram lista com marcador; uma linha so fica normal
  function fmtServicoWhats(s){
    const linhas=String(s||'').split('\n').map(x=>x.trim()).filter(Boolean)
    if(linhas.length===0) return 'Servico: -'
    if(linhas.length===1) return 'Servico: '+linhas[0]
  ...
  function EditField({campo,label,type,textarea,gridFull}){
    return <div style={gridFull?{gridColumn:'1/-1'}:{}}>
      ...
      {editando
        ...
        :<span style={{color:'#333',whiteSpace:'pre-wrap'}}>{type==='number'?fmt(form[campo]):(form[campo]||'-')}</span>}
    </div>
  }
                ...
                <div class="section" style={{marginBottom:16,padding:14,background:'#f9f9f7',borderRadius:8,border:'1px solid #eee'}}>
                  ...
                  <div style={{marginBottom:8,fontSize:13}} className="full">
                    ...
                    {editando
                      ...
                      : <div style={{color:'#333',whiteSpace:'pre-wrap',marginTop:2}}>{form.servico||'-'}</div>}
                  </div>
