...
export default function Clientes() {
  ...
  return (
    <Layout title="Clientes">
      ...
      {histModal && (
        <div style={s.overlay}>
          <div style={{...s.modal, width:620, maxHeight:'85vh', overflow:'auto'}}>
            <div style={s.mHead}>
              <span>Histórico — {histModal.nome}</span>
              <button style={s.xBtn} onClick={()=>setHistModal(null)}>×</button>
            </div>

            {/* DADOS DO CLIENTE */}
            <div style={s.infoBox}>
              <div style={s.infoRow}><span style={s.infoLabel}>Telefone:</span> {histModal.telefone||'—'}</div>
              <div style={s.infoRow}><span style={s.infoLabel}>Endereço:</span> {histModal.endereco||'—'}</div>
              {histModal.email && <div style={s.infoRow}><span style={s.infoLabel}>Email:</span> {histModal.email}</div>}
              {histModal.cpf_cnpj && <div style={s.infoRow}><span style={s.infoLabel}>CPF/CNPJ:</span> {histModal.cpf_cnpj}</div>}
              {histModal.observacoes && <div style={s.infoRow}><span style={s.infoLabel}>Obs:</span> {histModal.observacoes}</div>}
            </div>

            {/* ORDENS DE SERVIÇO */}
            <div style={s.secTitle}>Ordens de Serviço ({hist.os.length})</div>
            {hist.os.length === 0 && <div style={s.empty}>Nenhuma OS encontrada.</div>}
            {hist.os.map(o=>(
              <div key={o.id} style={s.histCard}>
                <div style={s.histCardHead}>
                  <div>
                    <span style={s.histNum}>OS #{o.numero}</span>
                    {o.produto && <span style={s.histProd}> · {o.produto}</span>}
                  </div>
                  <div style={{display:'flex',alignItems:'center',gap:8}}>
                    ...
                    <span style={{fontSize:12,fontWeight:600,color:'#1D9E75'}}>{fmt(o.valor)}</span>
                  </div>
                </div>
                <div style={s.histGrid}>
                  <div><span style={s.infoLabel}>Data entrada:</span> {fmtDate(o.data_entrada)}</div>
                  <div><span style={s.infoLabel}>Data conclusão:</span> {fmtDate(o.data_conclusao)}</div>
                ...
                {o.descricao && <div style={s.histDesc}><span style={s.infoLabel}>Diagnóstico:</span> {o.descricao}</div>}
                {o.observacoes && <div style={s.histDesc}><span style={s.infoLabel}>Observações:</span> {o.observacoes}</div>}
                {o.alterada && <div style={{fontSize:11,color:'#854F0B',marginTop:4}}>⚠️ OS alterada após emissão de recibo</div>}
              </div>
            ))}

            {/* AGENDAMENTOS */}
            <div style={{...s.secTitle, marginTop:16}}>Agendamentos ({hist.agendamentos.length})</div>
            {hist.agendamentos.length === 0 && <div style={s.empty}>Nenhum agendamento encontrado.</div>}
            {hist.agendamentos.map(a=>(
              <div key={a.id} style={{...s.histCard, borderLeftColor:'#B5D4F4'}}>
                <div style={s.histCardHead}>
                  <div style={{fontWeight:500}}>{a.servico}</div>
                  <span style={{display:'inline-block',padding:'2px 8px',borderRadius:999,fontSize:11,fontWeight:500,background:'#E6F1FB',color:'#185FA5'}}>{a.status}</span>
                </div>
                <div style={s.histGrid}>
                  <div><span style={s.infoLabel}>Data:</span> {fmtDate(a.data)} {a.hora ? '· '+a.hora.slice(0,5) : ''}</div>
                  <div><span style={s.infoLabel}>Funcionário:</span> {a.usuarios?.nome||'—'}</div>
                </div>
                {a.observacoes && <div style={s.histDesc}><span style={s.infoLabel}>Obs:</span> {a.observacoes}</div>}
              </div>
            ))}
...
const s = {
  ...
  infoBox:{background:'#f9f9f7',borderRadius:8,padding:'10px 14px',marginBottom:16,fontSize:13},
  ...
  histCardHead:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8},
  histNum:{fontWeight:700,fontSize:13},
  histProd:{fontSize:12,color:'#888'},
  histGrid:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6,fontSize:12,color:'#555',marginBottom:6},
  histDesc:{fontSize:12,color:'#666',marginTop:4},
}
