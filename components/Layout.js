...
import { useTheme, ACCENT_LIST, GRADIENTES, grad } from '../lib/theme'
...
function Icon({name, size=18}) {
  ...
  const paths = {
    ...
    etiqueta:<><path d="M20.6 13.4L13.4 20.6a2 2 0 0 1-2.8 0l-7.2-7.2A2 2 0 0 1 3 12V5a2 2 0 0 1 2-2h7a2 2 0 0 1 1.4.6l7.2 7.2a2 2 0 0 1 0 2.6z"/><circle cx="7.5" cy="7.5" r="1.3"/></>,
...
// Estilos que inline nao faz: :hover, transicao e animacao.
// Bloco minimo injetado direto — sem framework e sem dependencia nova.
function EstilosGlobais({t}) {
  const css = `
    *{-webkit-tap-highlight-color:transparent}
    .sg-tab{transition:background .28s ease,padding .28s ease,box-shadow .28s ease,color .2s ease}
    .sg-tab:active{transform:scale(.94)}
    .sg-nav{transition:background .18s ease,color .18s ease,transform .18s ease}
    .sg-nav:hover{background:${t.bgHover};transform:translateX(2px)}
    .sg-card{transition:transform .2s ease,box-shadow .2s ease}
    .sg-card:hover{transform:translateY(-2px);box-shadow:${t.shadowFloat}}
...
export default function Layout({ children, title = 'Dashboard' }) {
  ...
  const navGestor = [
    ...
    { href: '/etiquetas', icon: 'etiqueta', label: 'Etiquetas', grupo: 'Operação' },
  ...
  // icone/gradiente da secao aberta agora
  const paginaAtual = (nav.find(i => i.href === router.pathname) || nav[0]).icon
    ...
    return (
      <div style={{display:'flex',flexDirection:'column',minHeight:'100vh',background:t.bg,color:t.text}}>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:10,padding:'12px 16px',
                     background:t.dark?'rgba(20,19,16,.85)':'rgba(255,255,255,.82)',
                     backdropFilter:'blur(18px)',WebkitBackdropFilter:'blur(18px)',
                     borderBottom:'1px solid '+t.borderSoft,position:'sticky',top:0,zIndex:50}}>
          <div style={{display:'flex',alignItems:'center',gap:10,minWidth:0}}>
            <div style={{width:36,height:36,borderRadius:11,background:grad(paginaAtual),display:'flex',alignItems:'center',
                         justifyContent:'center',color:'#fff',flexShrink:0,boxShadow:'0 6px 14px -4px '+(GRADIENTES[paginaAtual]||GRADIENTES.dashboard)[1]+'99'}}>
              <Icon name={paginaAtual} size={19}/>
            </div>
            <div style={{minWidth:0}}>
              <div style={{fontSize:15,fontWeight:700,color:t.text,lineHeight:1.15,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{title}</div>
              <div style={{fontSize:10.5,color:t.textSoft}}>Top Eletro · Inova</div>
            </div>
          </div>
          <div style={{display:'flex',alignItems:'center',gap:2,flexShrink:0}}>
            {isGestor&&<NotificacoesSino t={t}/>}
            <button className="sg-btn" style={{background:'none',border:'none',cursor:'pointer',color:t.textSoft,padding:6,display:'flex'}} onClick={()=>setMenuAberto(!menuAberto)}>
              <Icon name={menuAberto?'fechar':'menu'} size={22}/>
            </button>
          </div>
        </div>
        {menuAberto&&(
          <div style={{position:'fixed',inset:0,zIndex:99,background:'rgba(0,0,0,.45)',backdropFilter:'blur(3px)'}} onClick={()=>setMenuAberto(false)}>
            <div className="sg-fade" style={{position:'absolute',top:0,right:0,width:270,height:'100%',background:t.bgSidebar,borderLeft:'1px solid '+t.border,padding:'16px 12px',display:'flex',flexDirection:'column',overflow:'auto'}} onClick={e=>e.stopPropagation()}>
              {nav.map(item=>{
                const on = router.pathname===item.href
                return (
                  <Link key={item.href} href={item.href} className="sg-nav" style={{display:'flex',alignItems:'center',gap:12,padding:'10px 10px',borderRadius:12,fontSize:14,fontWeight:on?700:500,color:on?t.text:t.textSoft,background:on?t.bgHover:'transparent',marginBottom:3,textDecoration:'none'}}>
                    <span style={{width:34,height:34,borderRadius:10,flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',
                                  background:on?grad(item.icon):t.bgHover,color:on?'#fff':t.textSoft,
                                  boxShadow:on?'0 6px 14px -5px '+(GRADIENTES[item.icon]||GRADIENTES.dashboard)[1]+'99':'none'}}>
        ...
        {/* BARRA INFERIOR — pilula com gradiente na secao ativa */}
        <div style={{position:'fixed',bottom:0,left:0,right:0,zIndex:50,padding:'8px 10px calc(8px + env(safe-area-inset-bottom))',
                     background:t.dark?'rgba(20,19,16,.88)':'rgba(255,255,255,.86)',
                     backdropFilter:'blur(18px)',WebkitBackdropFilter:'blur(18px)',
                     borderTop:'1px solid '+t.borderSoft}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-around',gap:6}}>
            {navMobile.map(item=>{
              const on = router.pathname===item.href
              const g = GRADIENTES[item.icon]||GRADIENTES.dashboard
              return (
                <Link key={item.href} href={item.href} className="sg-tab" data-on={on?'1':'0'}
                  style={{position:'relative',display:'flex',alignItems:'center',justifyContent:'center',gap:8,
                          height:48,padding:on?'0 16px':'0 14px',borderRadius:999,flexShrink:0,
                          background:on?grad(item.icon):'transparent',
                          boxShadow:on?'0 8px 20px -6px '+g[1]+'88':'none',
                          color:on?'#fff':t.textSoft,textDecoration:'none'}}>
                  <Icon name={item.icon} size={on?21:22}/>
                  {on&&<span style={{fontSize:12.5,fontWeight:700,whiteSpace:'nowrap',letterSpacing:'.01em'}}>{item.label}</span>}
                </Link>
              )
            })}
          </div>
  ...
  const s = {
    ...
    navItem:{display:'flex',alignItems:'center',gap:10,padding:'6px 8px',borderRadius:12,fontSize:13,color:t.textSoft,textDecoration:'none'},
    navActive:{background:t.bgHover,color:t.text,fontWeight:700},
    ...
    topbar:{padding:'12px 22px',borderBottom:'1px solid '+t.borderSoft,display:'flex',alignItems:'center',justifyContent:'space-between',gap:14,flexShrink:0,
            background:t.dark?'rgba(20,19,16,.85)':'rgba(255,255,255,.82)',backdropFilter:'blur(18px)',WebkitBackdropFilter:'blur(18px)',position:'sticky',top:0,zIndex:20},
  ...
  return (
    <div style={s.app}>
          ...
          {nav.map((item,i)=>{
            ...
            return (
              <div key={item.href} style={{display:'flex',flexDirection:'column',gap:2}}>
                ...
                <Link href={item.href} className="sg-nav" style={{...s.navItem,...(on?s.navActive:{})}}>
                  <span style={{width:30,height:30,borderRadius:9,flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',
                                background:on?grad(item.icon):(t.dark?t.bgHover:t.bg),color:on?'#fff':t.textSoft,
                                boxShadow:on?'0 6px 14px -5px '+g[1]+'99':'none'}}>
      ...
      <div style={s.main}>
        ...
        <div style={s.content} className="sg-fade" key={router.pathname}>{children}</div>
      </div>
    </div>
  )
}
